...
def processDemo():
    for filename in os.listdir(directoryPath):
        if "DEMO" in filename.upper() and "TXT" in filename.upper():
            ...
            demo_df = pd.read_csv(directoryPath + "/" + filename, low_memory=False, sep='$', on_bad_lines='skip')
            ...
            demo_df.drop(
                ['caseversion', 'i_f_code', 'lit_ref', 'event_dt', 'auth_num', 'fda_dt', 'age_grp', 'e_sub',
                 ...
                 'mfr_sndr', 'occp_cod'], inplace=True, axis=1, errors='ignore')
...
def processDrug():
    for filename in os.listdir(directoryPath):
        if "DRUG" in filename.upper() and "TXT" in filename.upper():
            print("Process " + filename)
            drug_df = pd.read_csv(directoryPath + "/" + filename, low_memory=False, sep="$", on_bad_lines='skip')

            # keep primaryid, caseid, role_cod, drugname
            drug_df.drop(
                ['drug_seq', 'val_vbm', 'dose_vbm', 'dose_form', 'dose_amt', 'dose_unit', 'cum_dose_chr', 'prod_ai',
                 'cum_dose_unit', 'dechal', 'rechal', 'lot_num', 'exp_dt', 'nda_num', 'dose_freq'],
...
def processReac():
    for filename in os.listdir(directoryPath):
        if "REAC" in filename.upper() and "TXT" in filename.upper():
            print("Process " + filename)
            reac_df = pd.read_csv(directoryPath + "/" + filename, low_memory=False, sep="$", on_bad_lines='skip')
...
def processOutc():
    for filename in os.listdir(directoryPath):
        if "OUTC" in filename.upper() and "TXT" in filename.upper():
            print("Process " + filename)
            outc_df = pd.read_csv(directoryPath + "/" + filename, low_memory=False, sep="$", on_bad_lines='skip')
